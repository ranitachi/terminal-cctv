...
foreach ($fd as $kf => $vff) {
	...
	if( $files['acsm'] != null)
	{
		...
		{
			krsort($d_acsm);
			$acsms = array_slice($d_acsm, 0,1,true);
			// $asf_array[]=$acsms;
			foreach ($acsms as $k => $acsm) {
				...
				if(file_exists($dirtarget.$asf_name) == false)
				{
					...
					convertAcsmToAsf($acsm, $dirtarget);
					// echo 'konversi acsm ke asf'."\n";
					/*$dur = $getID3->analyze($asf);
					$durasi=$dur['playtime_string'];
					//$mp4_array[]=$asf_name.'-'.$durasi['playtime_string'];
					//echo $asf_name.' : '.$durasi['playtime_string'];
					if(file_exists($dirtarget.$mp4_name) == false)
					{
						$d=explode(':', $durasi);
						if(count($d)==2)
						...
						if($total>=300)
						{
							// echo $namafile.'--'.$durasi.'--'.kurangwaktu(date('Y-m-d').' '.$cdur,2);
							$bagi=300/60;
							for($i=$bagi;$i>=1;$i--)
							{
								// $cdur.'';
								$n=convertAsfToMp42($asf,$namafile, $mp4,$cdur,(60*$i));
								$mp4_array[]=$mp4_name;
								$nn=explode('\\', $n);
								$new=$nn[count($nn)-1];
								rename($n, $dirtarget.$new);
								// list($n_file,$ext_file)=explode('.', $new);
								// $new_mp4=
							}
							// getThumb($mp4,$jpg);
						...
						else
						{
							...
							{
							// convertAsfToMp4($asf, $mp4);
							// getThumb($mp4,$namafile);
								$n=convertAsfToMp42($asf,$namafile, $mp4,$cdur,(60*$i));
								echo 'Bikin Thumb Sudah<br>';
								$mp4_array[]=$mp4_name;
								$nn=explode('\\', $n);
								$new=$nn[count($nn)-1];
								rename($n, $dirtarget.$new);
