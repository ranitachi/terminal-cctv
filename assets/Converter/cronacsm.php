...
foreach ($fd as $kf => $vff) {
	...
	if( $files['acsm'] != null)
	{
		...
		while($files['acsm'])
		{
			...
			$acsm_name=dirname($acsm) . trim(' \ ') .strtolower(pathinfo($acsm, PATHINFO_FILENAME)) . ".acsm";
			...
			if(date('Y-m-d')==$nf1)
			{
				if($gettime<=$jm)
				{
					...
					if($getminute<=$mn)
					{
						$size=filesize($acsm_name);
						$mp4_array[]='1 -- '.$vff.'--['.formatSizeUnits($size).']-'.$namafile.'.acsm';
		...
		if(empty($mp4_array_before))
		{
			...
			foreach ($acsms as $k => $acsm) {
				...
				$size=filesize($file_acsm);
				$mp4_array[]='2 -- '.$vff.'--['.formatSizeUnits($size).']-'.$namafile.'.acsm';	
			}
			// ksort($mp4_array);
